...
/**
 * test update api using Object array as parameters
 */
public class UpdateByArrayPerformance {
	...
	public static void run(int rowNumber, int loops) throws DBException, SQLException{
		...
		Object[] stu = makeStudents(rowNumber);
	 ...
	 * @throws DBException
	 */
	public static void rexdbUpdateByBean(Object[] stu) throws DBException{
		for (int i = 0; i < stu.length; i++) {
			DB.update(INSERT, stu[i]);
		}
	}
	
	private static Object[] makeStudents(int i){
		Object[] arrays = new Object[i];
		for (int j = 0; j < i; j++) {
			Object ps = new Object[]{"Jim", 1, new Date(), new Date(), new Date(), 10, null, null, 1};
			arrays[j] = ps;
		}
		return arrays;
	}
