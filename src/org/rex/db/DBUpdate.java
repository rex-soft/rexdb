...
public class DBUpdate extends DBOperation {
	 ...
	 * (1) if the given parameter 'parameters' is <tt>null</tt>, SQL will be executed in no-prepared <tt>Statement</tt> object,
	 * otherwise in <tt>PreparedStatement</tt> object.<br/>
	 * (2) if the given parameter 'parameters' is a <tt>Ps</tt> object, SQL uses '?' representing prepared parameters, and sets
	 * parameters according to the Ps built-in order.<br/>
	 * (3) if the given parameter 'parameters' is an <tt>Array</tt>, such as a String array, SQL uses '?' representing prepared
	 * parameters, and sets parameters according to the array elements order.<br/>
	 * (4) if the given parameter 'parameters' is a <tt>Map</tt>, SQL uses '#{ <i>parameter name</i>}' representing prepared
	 * parameters('parameter name' is a Map's entry key, and the entry value is the corresponding prepared parameter value). if
	 * the Map doesn't contains key 'parameter name', the prepared parameter value will be set to <tt>null</tt>.<br/>
	 * (5) if the given parameter 'parameters' is a java bean, SQL uses '#{ <i>parameter name</i>}' representing prepared
	 * parameters('parameter name' is a java bean property, which has a readable method), if java bean doesn't have property
	 ...
	 *         that return nothing
	 * @throws DBException either rexdb configuration wasn't loaded, or database access error occurs, or couldn't execute SQL,
	 *             etc. the specific reason will be included in the exception message.
	 */
	 ...
	 * @return an array of update counts containing one element for each command in the batch.
	 * @throws DBException if rexdb configuration wasn't loaded, database access error occurs, couldn't execute SQL, etc. the
